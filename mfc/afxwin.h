...
#undef CopyRgn
#else
// We do need these from WINDOWSX.H
...
// Constructors
    ...
    // create from a point
    CSize(POINT initPt);
    // create from a DWORD: cx = GET_X_LPARAM(dw) cy = GET_Y_LPARAM(dw)
    CSize(DWORD dwSize);
...
// Constructors
    ...
    // create from a dword: x = GET_X_LPARAM(dw) y = GET_Y_LPARAM(dw)
...
// Window Text Functions
    void SetWindowText(__in_z_opt LPCTSTR lpszString);
...
// Attributes
    ...
    int GetLine(__in int nIndex, 
                __inout 
                __pre __byte_readableTo(sizeof(WORD))
                __pre __elem_writableTo(*((LPWORD)lpBuffer))
                __post __elem_readableTo(return)
                LPTCH  lpBuffer) const;

    int GetLine(__in int nIndex, 
                __out_ecount_part(nMaxLength, return) LPTCH lpBuffer, 
                __in int nMaxLength) const;
...
#if defined(_AFX_DLL_IMPL) && defined(_WIN64)
	__declspec(noinline) virtual ~CMDIFrameWnd() { }
#endif		
...
#if defined(_AFX_DLL_IMPL) && defined(_WIN64)
	__declspec(noinline) virtual ~CMDIChildWnd() { }
#endif	
...
BOOL AFXAPI AfxWinInit(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    __in_z LPTSTR lpCmdLine, int nCmdShow);
...
void AFXAPI AfxFormatStrings(CString& rString, LPCTSTR lpszFormat,
                            LPCTSTR const* rglpsz, int nString);
BOOL AFXAPI AfxExtractSubString(CString& rString, __in_z_opt LPCTSTR lpszFullString,
